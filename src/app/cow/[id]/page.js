...
async function getCow(id) {
  const response = await fetch(`https://mattel-back.onrender.com/api/cows/${id}?populate=img`, { next: { revalidate: 60 } })
...
const cowPage = async function ({ params: {id} }) {
  ...
  return (
    ...
    <Link href="/" className="text-white border-white border-2 rounded-md p-4 my-4">🐄 Go Back to tail...</Link>
      <div className='flex w-full flex-col items-center justify-around'>
        ...
        <p className='text-white my-4 p-4 text-xl'> { description } </p>
        <i className='text-white'>born at: {born}</i>
      </div>
    </div>
  )
}
